...
/// @return All class and instance methods.
+ (NSArray<MKMethod*> *)allMethods;
...
/// Using the \c MKTypeEncoding function for the same method looks like this:
/// @code MKTypeEncodingString(@encode(void), 1, @encode(NSUInteger)) @endcode
/// @param typeEncoding The type encoding string. Consider using the \c MKTypeEncodingString() function.
/// @param classMethod NO to add the method to the class itself or YES to add it as an instance method.
/// @return YES if the method was added successfully, \c NO otherwise
/// (for example, the class already contains a method implementation with that name).
+ (BOOL)addMethod:(SEL)selector typeEncoding:(NSString *)typeEncoding implementation:(IMP)implementaiton toInstances:(BOOL)instanceMethod;
...
+ (NSArray<MKProperty*> *)allProperties;
